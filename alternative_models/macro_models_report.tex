...
\begin{itemize}[nosep, leftmargin=1.2em]
    \item \textbf{Point forecast above consensus:} Both the Ridge point estimate (126.7K) and QR median (177.3K) sit above the 95K consensus. The model's three inputs explain this: (i) lagged NFP of +178K in March provides positive AR momentum; (ii) initial claims remained low ($\sim$200K weekly average), signalling continued hiring; (iii) VIX moderated from its Iran-war peak, implying easing financial conditions.
    \item \textbf{Wide 80\% CI [$-$64K, 302K]:} The interval spans from outright contraction to strong expansion. This is not a model failure---it reflects the irreducible noise in the establishment survey. NFP revisions routinely exceed 50K, and the BLS birth-death model adds further unpredictability. The 4.7\% improvement over naive is the ceiling of what is achievable without proprietary data (ADP micro, tax withholdings).
    \item \textbf{Probability skew:} P(inline)=58\% dominates, with P(beat)=24.9\% outweighing P(miss)=17.1\%. The asymmetry reflects the strong claims signal: when weekly claims are below 220K, NFP prints below 50K occur less than 15\% of the time historically.
    \item \textbf{Why Ridge wins over XGBoost:} With only 116 monthly observations, tree-based models overfit even at depth=2. Ridge's L2 penalty shrinks coefficients toward zero uniformly, acting as an implicit Bayesian prior. The minimal 3-feature set eliminates the curse of dimensionality entirely---each feature has 39 observations per parameter.
\end{itemize}
...
\begin{itemize}[nosep, leftmargin=1.2em]
    \item \textbf{NFP above consensus + Unemployment below consensus} ($\rightarrow$ tighter labour market): The NFP model sees 127K vs 95K consensus; the unemployment model forecasts 4.05\% vs 4.3\% consensus (P(miss)=60\%). Both independently point to a labour market that is stronger than the market expects.
    \item \textbf{Stronger hiring + Higher wage growth} ($\rightarrow$ Phillips curve channel): The AHE model forecasts 0.41pp MoM vs 0.2pp consensus (P(beat)=67\%). A tighter labour market (from NFP/unemployment signals) mechanically supports stronger wage bargaining---exactly what the AHE model detects.
    \item \textbf{Implication for rates:} The three US labour signals are unanimously hawkish. If realised, this combination would: (i) push Fed rate cut expectations further out, (ii) steepen the front end of the Treasury curve (higher terminal rate), and (iii) spill into global duration via the VAR channels identified in our spillover analysis---particularly UK gilts (29\% of Level variance from US) and Canadian bonds (47\% of Slope variance from US).
    \item \textbf{Brazil IPCA miss as a partial offset:} The BR model (P(miss)=79\%) provides a disinflationary counterpoint in EM. A below-consensus IPCA print would ease pressure on the BCB and compress DI rates, partially offsetting the hawkish US impulse for global risk sentiment.
\end{itemize}
