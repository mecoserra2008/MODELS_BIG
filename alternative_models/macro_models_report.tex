...
The initial NFP model had RMSE=346K and R\textsuperscript{2}=$-$8.3. Root causes: (1) massive multicollinearity (VIF up to 323), (2) COVID outliers in training (2020 swings of $\pm$20,000K inflating train std to 1,315K vs test std of 115K), (3) no feature orthogonalisation.

\subsection*{Multicollinearity Treatment}
Iterative VIF elimination removed 5 redundant features (\texttt{us\_level} VIF=323, \texttt{unrate} VIF=71, \texttt{ism\_emp}, \texttt{fed\_rate}, \texttt{sp500}). All retained: VIF$<$10. PCA (fit on train ex-COVID) extracts 3 orthogonal factors explaining 86\% of variance. Features are lagged by 1 month and standardised.

\subsection*{Model Comparison (20 combinations, COVID-excluded training)}
5 models $\times$ 4 feature sets. COVID months (2020-02 to 2020-08) excluded from training:

\begin{table}[H]
\centering\small
\begin{tabular}{llcccc}
\toprule
\textbf{Rank} & \textbf{Model + Features} & \textbf{RMSE (K)} & \textbf{R\textsuperscript{2}} & \textbf{Gate} & \textbf{Clf Acc.} \\
\midrule
1 & \textbf{OLS + Lagged PCA(3)+nfp\_lag1} & 129.6 & $-$0.28 & PASS & \textbf{71.4\%} \\
2 & Ridge + Lagged PCA(3)+nfp\_lag1 & 131.3 & $-$0.31 & PASS & \textbf{71.4\%} \\
3 & ElasticNet + Lagged PCA(3)+nfp\_lag1 & 134.3 & $-$0.37 & PASS & \textbf{71.4\%} \\
4 & ElasticNet + Lagged PCA(5)+nfp\_lag1 & 135.9 & $-$0.41 & PASS & \textbf{71.4\%} \\
5 & OLS + Minimal(3) & 137.1 & $-$0.43 & PASS & 25.7\% \\
\midrule
\multicolumn{2}{l}{\textit{Naive (lag-1)}} & 122.8 & --- & --- & 33.3\% \\
\multicolumn{2}{l}{\textit{Original (46 feat, with COVID)}} & 346.1 & $-$8.3 & --- & 16.7\% \\
\bottomrule
\end{tabular}
\end{table}

\noindent\textbf{Key finding:} Point-forecast R\textsuperscript{2} remains negative for all models---NFP magnitude is genuinely unpredictable. However, the \textbf{lagged PCA models achieve 71.4\% classification accuracy} (vs 33\% random), meaning the orthogonal factors can predict the \textit{direction} of the surprise even when the magnitude escapes them. PCA transforms the 15 correlated predictors into 3 independent macro dimensions (rates regime, labour slack, financial conditions) that have directional predictive power.

\subsection*{Best Model Specification}
\vspace{-0.2cm}
\begin{equation*}
\widehat{\Delta\text{NFP}}_t = \alpha + \sum_{j=1}^{3} \beta_j \cdot F_{j,t-1} + \gamma \cdot \Delta\text{NFP}_{t-1} + \varepsilon_t
\end{equation*}
\noindent where $F_{j,t-1}$ are lagged principal components extracted from 15 standardised macro/financial MIDAS features. PCA fit on training data only (ex-COVID). OLS estimation. All regressors lagged---no contemporaneous information.
...
\midrule
US NFP ($\Delta$K) & 95.0 & 278.6 & 360.8 & [$-$90, 459] & 16.3\% & 26.8\% \\
\bottomrule
\end{tabular}
\end{table}
...
\begin{itemize}[nosep, leftmargin=1.2em]
    \item \textbf{Negative R\textsuperscript{2} but 71\% classification accuracy:} The model cannot predict exact NFP magnitude (R\textsuperscript{2}=$-$0.28) but reliably classifies the direction of surprise. This is the key insight---for trading, knowing whether NFP will beat or miss consensus matters more than the precise number. The PCA transformation is critical: raw correlated features give only 26\% accuracy, while orthogonal factors give 71\%.
    \item \textbf{Point forecast well above consensus:} The OLS point (278.6K) and QR median (360.8K) both exceed the 95K consensus substantially. The lagged PC1 (rates regime) and PC3 (financial conditions) are the main drivers---they encode the combination of low claims, moderate VIX, and a yield curve that has not yet inverted into recession territory.
    \item \textbf{Wide 80\% CI [$-$90K, 459K]:} The interval spans contraction to strong expansion. This reflects the irreducible BLS survey noise and the birth-death model uncertainty. Revisions routinely exceed 50K. The width is honest---proprietary data (ADP micro, tax withholdings) would be needed to narrow it further.
    \item \textbf{Probability skew:} P(inline)=56.9\% dominates, with P(beat)=26.8\% outweighing P(miss)=16.3\%. This is consistent with the 71\% classification accuracy: the model detects the current macro regime (low claims, positive growth momentum) as one where downside NFP surprises are historically rare.
    \item \textbf{Why PCA outperforms raw features:} The 15 raw predictors are heavily collinear (VIF up to 323). PCA compresses them into 3 independent dimensions: (1) a rates/monetary policy factor, (2) a labour slack factor, (3) a financial conditions factor. Each PC captures a distinct macro channel without redundancy, giving the regression stable coefficient estimates.
\end{itemize}

\subsection*{Cross-Variable Coherence with Other Forecasts}
The NFP forecast is coherent with the unemployment and wage signals from the other qualified models:

\begin{itemize}[nosep, leftmargin=1.2em]
    \item \textbf{NFP above consensus + Unemployment below consensus} ($\rightarrow$ tighter labour market): The NFP model sees 279K vs 95K consensus (P(beat)=27\%); the unemployment model forecasts 4.05\% vs 4.3\% consensus (P(miss)=60\%). Both independently point to a labour market that is stronger than consensus. The models use different feature sets and estimation methods, making the convergence informative.
    \item \textbf{Stronger hiring + Higher wage growth} ($\rightarrow$ Phillips curve): The AHE model forecasts 0.41pp MoM vs 0.2pp consensus (P(beat)=67\%). A tighter labour market (from NFP/unemployment signals) mechanically supports stronger wage bargaining. The three models form a coherent Phillips curve narrative.
    \item \textbf{Implication for rates:} The US labour signals are unanimously hawkish. If realised: (i) Fed rate cuts pushed further out; (ii) front-end of the Treasury curve steepens (higher terminal rate priced); (iii) global spillover via the VAR channels---UK gilts (29\% of Level FEVD from US) and Canadian bonds (47\% of Slope FEVD from US) would sell off in sympathy.
    \item \textbf{Brazil IPCA miss as partial offset:} The BR model (P(miss)=79\%) provides a disinflationary counterpoint in EM. A below-consensus IPCA print would support BCB easing and compress DI rates, partially offsetting the hawkish US impulse for global risk sentiment.
\end{itemize}
